...
function negate(vec: Vector): Vector {
    return { x: -vec.x, y: -vec.y };
}

function dot(v0: Vector, v1: Vector): number {
...
function scale(vec: Vector, s: number): Vector {
...
function normal(vec: Point): Point {
...
function isEqual(v0: Vector, v1: Vector): boolean {
    return v0.x == v1.x && v0.y == v1.y;
}
...
function generateFlightPath(
    ...
    let parallelVec: Vector = { x: Math.sin(rad), y: Math.cos(rad) };

    // get vector perpendicular to strips
    let perpVec: Vector = perp(parallelVec);
    if (turn === 1) {
    ...
    const rowEnds: { start: Vector, end: Vector }[] = [];
    ...
    for (let i = 0; i < rowEnds.length; i++) {
        const row = rowEnds[i];
        let closestHit: Vector | undefined;
        let furthestHit: Vector | undefined;
